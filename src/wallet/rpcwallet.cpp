...
#include "wallet/asyncrpcoperation_shield.h"
...
UniValue z_shield(const UniValue& params, bool fHelp)
{
    if (!EnsureWalletIsAvailable(fHelp))
        return NullUniValue;
    ...
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
                ...
                + HelpRequiringPassphrase() + "\n"
                "\nArguments:\n"
                "1. \"amounts\"             (array, required) An array of json objects representing the amounts to send.\n"
                "    [{\n"
                "      \"address\":address  (string, required) The address is a taddr or zaddr\n"
                "      \"amount\":amount    (numeric, required) The numeric amount in ZEC is the value\n"
                "      \"memo\":memo        (string, optional) If the address is a zaddr, raw data represented in hexadecimal string format\n"
                "    }, ... ]\n"
                "2. minconf               (numeric, optional, default=1) Only use funds confirmed at least this many times.\n"
