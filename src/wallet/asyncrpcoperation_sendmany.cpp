...
#include "asyncrpcoperation_shield.h"
...
if (LogAcceptCategory("zrpcunsafe")) {
    ...
    LogPrint("zrpc", "%s: z_shield initialized\n", getId());
}
}
...
void AsyncRPCOperation_shield::main() {
if (isCancelled())
    return;
...
std::string s = strprintf("%s: z_shield finished (status=%s", getId(), getStateAsString());
if (success) {
    s += strprintf(", txid=%s)\n", tx_.GetHash().ToString());
} else {
    s += strprintf(", error=%s)\n", getErrorMessage());
...
//if (isfromtaddr_) {
    ...
    CAmount funds = selectedUTXOAmount;
//CAmount fundsSpent = t_outputs_total + minersFee + z_outputs_total;
    CAmount fundsSpent =  minersFee + z_outputs_total;
    ...
    if (change > 0) {
        if (selectedUTXOCoinbase) {
            ...
            throw JSONRPCError(RPC_WALLET_ERROR, strprintf(
                "Change %s not allowed. When protecting coinbase funds, the wallet does not "
                "allow any change as there is currently no way to specify a change address "
                "in z_shield.", FormatMoney(change)));
        } else {
            add_taddr_change_output_to_tx(change);
            LogPrint("zrpc", "%s: transparent change in transaction output (amount=%s)\n",
                    getId(),
...
* Raw transaction as hex string should be in object field "rawtxn"
*/
void AsyncRPCOperation_shield::sign_send_raw_transaction(UniValue obj)
{   
// Sign the raw transaction
UniValue rawtxnValue = find_value(obj, "rawtxn");
if (rawtxnValue.isNull()) {
...
bool AsyncRPCOperation_shield::find_utxos(bool fAcceptCoinbase=false) {
//    set<CBitcoinAddress> setAddress = {fromtaddr_};
vector<COutput> vecOutputs;
...
*/
//This is the part that insists no a certain address that is not needed here
        /*             if (!setAddress.count(address)) {
            continue;
        }
      }
...
bool AsyncRPCOperation_shield::find_unspent_notes() {
std::vector<CNotePlaintextEntry> entries;
{
    LOCK2(cs_main, pwalletMain->cs_wallet);
    pwalletMain->GetFilteredNotes(entries, fromaddress_, mindepth_);
...
UniValue AsyncRPCOperation_shield::perform_joinsplit(AsyncJoinSplitInfo & info) {
std::vector<boost::optional < ZCIncrementalWitness>> witnesses;
uint256 anchor;
{
    LOCK2(cs_main, pwalletMain->cs_wallet);
...
UniValue AsyncRPCOperation_shield::perform_joinsplit(AsyncJoinSplitInfo & info, std::vector<JSOutPoint> & outPoints) {
std::vector<boost::optional < ZCIncrementalWitness>> witnesses;
uint256 anchor;
{
    LOCK(cs_main);
...
UniValue AsyncRPCOperation_shield::perform_joinsplit(
    AsyncJoinSplitInfo & info,
    std::vector<boost::optional < ZCIncrementalWitness>> witnesses,
    uint256 anchor)
{
...
void AsyncRPCOperation_shield::add_taddr_outputs_to_tx() {
...
void AsyncRPCOperation_shield::add_taddr_change_output_to_tx(CAmount amount) {
...
boost::array<unsigned char, ZC_MEMO_SIZE> AsyncRPCOperation_shield::get_memo_from_hex_string(std::string s) {
boost::array<unsigned char, ZC_MEMO_SIZE> memo = {{0x00}};
...
* Override getStatus() to append the operation's input parameters to the default status object.
*/
UniValue AsyncRPCOperation_shield::getStatus() const {
UniValue v = AsyncRPCOperation::getStatus();
if (contextinfo_.isNull()) {
    return v;
}
...
UniValue obj = v.get_obj();
obj.push_back(Pair("method", "z_shield"));
obj.push_back(Pair("params", contextinfo_ ));
return obj;
}
